#assembly and analysis of MPXV infection in HeLa FP dataset
#msglm run by Sabri
#Author: Yiqi Huang
###############################################################################
...
data_version_mpxv <- "20221130"
fit_version_mpxv <- "20221201"
...
load(file.path(scratch_path, paste0(project_id, '_HeLa_msglm_data_', data_version_mpxv, '.RData')))
load(file.path(scratch_path, paste0(project_id, '_HeLa_msdata_full_',  data_version_mpxv, '.RData')))
load(file.path(results_path, paste0(project_id, '_HeLa_msglm_fit_', fit_version_mpxv, '.RData')))
...
# pca for samples----
intensities_summary <- mpxv.env$msdata_full$protgroup_intensities %>% 
  mutate(log2_intensity = log2(intensity)) %>% 
  group_by(msexperiment_mq) %>% 
  summarise(average=mean(log2_intensity, na.rm = T),
            stdev = sd (log2_intensity, na.rm = T))

impute_intensities <- function(intensities_df, stats_df, log2_mean_offset=-1.8, log2_sd_scale=0.3, key = "msexperiment_mq"){
  inner_join(intensities_df, stats_df, by = key) %>%
    mutate(log2_intensity_imputed = if_else(is.na(log2_intensity),
                                            rnorm(n(), mean=average + stdev*log2_mean_offset,
                                                  sd=stdev * log2_sd_scale),
                                            log2_intensity))
} #Function adapted from Alexey

complete_intensities <- complete(mpxv.env$msdata_full$protgroup_intensities, protgroup_id, msexperiment_mq) %>% 
  mutate(log2_intensity = log2(intensity))

intensity_imputation <- impute_intensities(complete_intensities, intensities_summary) %>% 
  select(-c(average, stdev)) %>% 
  drop_na(log2_intensity_imputed)

intensity_PCA.mtx <- matrix(intensity_imputation$log2_intensity_imputed,
                            nrow = n_distinct(intensity_imputation$msexperiment_mq),
                            dimnames = list(msrun = unique(intensity_imputation$msexperiment_mq),
                                            protgroup_id = unique(intensity_imputation$protgroup_id)))

intensity_pca <- prcomp(intensity_PCA.mtx, scale. = T)

intensity_pca_df <- as_tibble(intensity_pca$x,
                              rownames="msrun") %>% 
  separate(msrun,c("Treatment","Time","Replicate"), remove = F) %>% 
  mutate(Time = str_remove(Time, "h"),
         Time = as.numeric(Time)) %>% 
  left_join(mpxv.env$msdata$msruns) %>% 
  arrange(rawfile) %>% 
  mutate(ID = seq(1:nrow(intensity_pca_df)))

ggplot(intensity_pca_df,
       aes(x=PC1, y=PC2,
           color = factor(Time)
       )) +
  geom_point(aes(shape = Treatment)) +
  geom_text(aes(label = Replicate), colour = "black", vjust = 1)+
  theme_bw()
...
object_contrasts_thresholds.df <- contrasts.df %>%
  mutate(p_value_threshold = case_when(TRUE ~ 0.001),
         ...
         median_threshold = ifelse(as.numeric(as.character(timepoint_lhs)) < 24, 0.125, 0.25)#,
...
object_contrasts.df <- fit_contrasts$object_conditions %>%
  ...
  mutate_at(c("contrast", "metacondition_rhs", "metacondition_lhs", "treatment_lhs") ,~str_replace_all(., "^mpx", "MPXV")) %>% 
  filter(timepoint_lhs == timepoint_rhs)
...
object_contrast_stats.df <- dplyr::group_by(object_contrasts.df, contrast, treatment_lhs, timepoint_lhs, contrast_type, ci_target) %>%
  ...
  arrange(timepoint_lhs)#%>% select(contrast, n_hits, n_plus, n_minus)
...
object_contrast_stats.df %>% select(contrast, treatment_lhs, timepoint_lhs, n_plus, n_minus) %>% 
  mutate(contrast = relevel(factor(contrast), ref = "MPXV_6h_VS_mock_6h")) %>% 
  ...
  ggplot(aes(contrast, plot_n, fill = change))+ 
...
write_tsv(object_contrasts_report.df,
          file.path(analysis_path, "reports", paste0(project_id, '_HeLa_MPXV_contrasts_report_', fit_version_mpxv, '_wide.txt')))
...
rfit_filepath <- file.path(results_path, paste0(project_id, '_HeLa_msglm_fit_', fit_version_mpxv, '.RData'))
results_info <- list(project_id = project_id, mstype = "fp",
                     data_version = data_version_mpxv, fit_version = fit_version_mpxv,
...
object_contrast_average.df <- object_contrasts.df %>% 
  filter(ci_target == "average") %>% 
  mutate(condition = str_replace(metacondition_lhs, "_", "@"),
         change = ifelse(is_hit, change, ".")) %>% 
  arrange(timepoint_lhs)
...
write_tsv(object_contrast_4paper.df,
          file.path(analysis_path, "reports", "sup_tables", paste0("Supplementary table X Total proteome of HeLa cells infected with MPXV.txt")))
