# Msglm fit chunk for the mpxv phospho data
...
rdata_filepath <- file.path(scratch_path, paste0(project_id, "_msglm_data_fp_", fit_version, ".RData"))
...
function(selobj_df) {
  ...
  stan_fit <- try_fit_model(model_data, 10L, adapt_delta=0.9,
                  ...
                  stanmodel_options=list(
                           ...
                           effect_slab_scale = 2, effect_slab_df = 12,
  ...
  if (inherits_any(stan_fit, "try-error")) {
    ...
    msglm_results <- process.stan_fit(stan_fit, model_data)
  } else {
...
rfit_filepath <- file.path(results_path, paste0(project_id, '_msglm_fit_phospho_', fit_version, '.RData'))
